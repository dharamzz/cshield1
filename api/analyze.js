...
async function fetchEthereumHolders(meta = {}) {
  // Use pre-fetched wallet data from eth_wallet.js
  // Refresh eth_wallet.js by running fetch_eth_info.html periodically (weekly recommended)
  // This gives consistent data every run — no live Ethplorer calls for ETH native

  if (!ETH_WALLET_DATA || ETH_WALLET_DATA.length === 0) {
    throw new Error(
      "ETH wallet data not available. Run fetch_eth_info.html to generate eth_wallet.js " +
      "and place it in the /api folder, then redeploy."
    );
  }

  // Get circulating supply + metadata from CoinGecko (still live — fast, single call)
  ...
  // Build holder objects from pre-fetched data
  ...
  if (!holders.length)
    throw new Error("ETH wallet data is empty. Please refresh eth_wallet.js using fetch_eth_info.html.");

  const generatedAt = ETH_WALLET_GENERATED_AT !== "placeholder"
    ? ` (data from ${new Date(ETH_WALLET_GENERATED_AT).toLocaleDateString()})`
    : "";
