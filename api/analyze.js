...
//  ANY coin     coinmap.js lookup            O(1) ticker→contract (zero CoinGecko calls)
//               + CoinGecko search           fallback for coins not in coinmap
//
...
async function resolveByName(query) {
  ...
  // coinmap.js maps 300+ tickers/names → contract + chain directly.
  // Metadata (price, image) fetched separately and is non-fatal.
  const mapped = coinLookup(query);
  if (mapped) {
    // Fetch optional metadata — failure is non-fatal
    const cgData = await cgMeta(mapped.cgId).catch(() => null);
    const meta = {
    ...
    // Native coin — route to dedicated fetcher
    if (!mapped.contract) {
      const id = mapped.cgId;
      ...
      // Unknown native — fall through to CoinGecko slow path below
    } else {
      // EVM token — go straight to holder fetch, skip CoinGecko entirely
      return fetchEVMTokenHolders(mapped.contract, mapped.chain, meta);
    }
  }

  // ── Slow path: unknown coin — 2 CoinGecko API calls ───────────────────────
  ...
  throw new Error(
    ...
    "and any EVM token on Arbitrum, Base, Optimism, Fantom, Cronos, Linea, zkSync + 40 more (requires MORALIS_API_KEY)."
  );
}
