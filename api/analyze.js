...
// Moralis chain name remapping — some chains use different IDs in Moralis API
const MORALIS_CHAIN_REMAP = {
  "polygon_zkevm": "polygon zkevm",
  "arbitrum_nova": "arbitrum nova",
};

// Chains not yet supported by Moralis free tier
// Coins on these chains return a clear message instead of HTTP 400
const MORALIS_UNSUPPORTED_CHAINS = new Set([
  "manta", "mode", "apechain", "astar", "boba", "canto", "fraxtal",
  "ink", "oasis", "okc", "soneium", "taiko", "tomo", "unichain",
  "worldchain", "xdc", "neon", "godwoken", "cube", "ethw", "dogechain",
  "milkomeda", "velas", "hoo", "bitgert", "wan", "step", "sx", "oasis",
  "energi", "fusion", "syscoin", "shiden", "meter",
]);

async function fetchEVMTokenHolders(address, moralisChain, meta = {}) {
  // Remap chain names where Moralis uses different identifiers
  const remapped = MORALIS_CHAIN_REMAP[moralisChain] || moralisChain;
  const chainHint = remapped === "bsc" ? "bsc" : "ethereum";
  const chainLabel = CHAIN_LABELS[moralisChain] || moralisChain;

  // Ethplorer covers ETH + BSC — try first for those two (no key needed)
  ...
  // Chains not supported by Moralis — return clear message immediately
  if (MORALIS_UNSUPPORTED_CHAINS.has(moralisChain)) {
    throw new Error(
      `Holder data for ${meta.name || address} on ${chainLabel} is not yet available. ` +
      `${chainLabel} is not currently supported by the Moralis API free tier.`
    );
  }

  // Moralis covers all remaining EVM chains
  try {
    ...
    // Last resort: try Ethplorer (has some cross-chain coverage)
    if (remapped !== "eth" && remapped !== "bsc") {
      try { return await fetchEthplorerByAddress(address, meta, moralisChain); } catch (_) {}
    }
    throw new Error(
      `Could not fetch holders for ${meta.name || address} on ${chainLabel}. ${e.message}`
    );
  }
}
