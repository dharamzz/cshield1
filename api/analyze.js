...
// WETH contract — used to fetch top holders as proxy for large unknown ETH holders
const WETH = "0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2";

// ─────────────────────────────────────────────────────────────────────────────
// KNOWN_ETH_WALLETS — 55 publicly known large ETH addresses
// Balances fetched live via Ethplorer, NOT hardcoded.
...
const KNOWN_ETH_WALLETS = [
  ...
  { address: "0xA9D1e08C7793af67e9d92fe308d5697FB81d3E43", label: "Coinbase Prime",                entity: "Exchange" },
  ...
  // ── Known Institutional / Foundations / Whales ────────────────────────────
...
async function fetchEthereumHolders(meta = {}) {
  ...
  // Step 1: Fetch live ETH balances sequentially to avoid Ethplorer rate limits.
  // Parallel calls (52 at once) cause most to 429 — sequential ensures all
  // wallets are fetched consistently and labels are preserved.
  ...
  for (const w of KNOWN_ETH_WALLETS) {
    ...
    } catch (_) { /* skip on error — continue with rest */ }
  ...
  // NOTE: Ethplorer getTopTokenHolders returns balance as raw token units (18 decimals)
  // so we must divide by 1e18 to convert to ETH.
  let wethHolders = [];
  try {
    const wethData = await fetchJSON(
      `https://api.ethplorer.io/getTopTokenHolders/${WETH}?apiKey=${KEY}&limit=50`
    );
    const knownAddrs = new Set(KNOWN_ETH_WALLETS.map(w => w.address.toLowerCase()));
    wethHolders = (wethData.holders || [])
      .filter(h => !knownAddrs.has(h.address.toLowerCase()))
      .map(h => ({
        address:    h.address,
        ethBalance: (h.balance || 0) / 1e18,   // convert raw units → ETH
        label:      null,
        entity:     "Whale",
        isContract: false,
      }));
  } catch (_) { /* non-fatal — known wallets cover major holders */ }

  // Step 3: Merge and deduplicate (known addresses take priority with their labels)
  const seen = new Set();
  const merged = [...knownHolders, ...wethHolders].filter(h => {
  ...
  // Step 4: Get circulating supply + metadata from CoinGecko
  ...
  // Step 5: Build holder objects, sort descending, return top 50
  const holders = merged
    .map(w => ({
      address:    w.address,
...
// All Etherscan-family explorers share ONE key from etherscan.io
...
async function fetchExplorerHolders(address, chain, meta = {}) {
  ...
  if (config.blockscout) {
    ...
    const d = await r.json();
    if (d.status === "0") {
      if ((d.result||"").includes("rate limit")) throw new Error("Explorer rate limit — try again shortly");
    ...
    return {
      ...
      holders: items.slice(0,50).map(h => {
        ...
        return { address:h.TokenHolderAddress,
