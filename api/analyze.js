...
async function fetchEthereumHolders(meta = {}) {
  ...
  if (!ETH_WALLET_DATA || ETH_WALLET_DATA.length === 0) {
    throw new Error(
      "ETH wallet data not loaded. Run fetch_eth_info.html to generate eth_wallet.js, " +
      "place it in /api folder, and redeploy."
    );
  }
  ...
  const generatedAt = ETH_WALLET_GENERATED_AT && ETH_WALLET_GENERATED_AT !== "placeholder"
