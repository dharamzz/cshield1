...
async function resolveByName(query) {
  ...
  if (mapped) {
    // NON-EVM coins — return unsupported message immediately, zero API calls
    if (mapped.type === "NON-EVM") {
      throw new Error(
        `Currently only Bitcoin and EVM-based coins are supported. ` +
        `"${query.toUpperCase()}" is a non-EVM coin and is not yet supported. ` +
  ...
  throw new Error(
    `Currently only Bitcoin and EVM-based coins are supported. ` +
    `"${detail.name}" (${detail.symbol?.toUpperCase()}) lives on ${chainStr}, which is not yet supported. ` +
    `Support for other chains will be added shortly.`
  );
}
