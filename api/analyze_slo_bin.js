...
// ─────────────────────────────────────────────────────────────────────────────
// SOLANA — Solscan public API (free, no key required)
//
// Works for both native SOL and any SPL token mint address.
//
// Native SOL:  Solscan /account/top-holders on the SOL mint
// SPL tokens:  Solscan /token/holders on the token mint address
//
// Fallback: Solana public RPC (mainnet-beta) via getTokenLargestAccounts
// which requires no key and returns the top 20 accounts for any SPL token.
// ─────────────────────────────────────────────────────────────────────────────
...
async function fetchSolanaHolders(mintAddress, meta = {}) {
  ...
  // ── Source 1: Solscan public API (free, no key) ───────────────────────────
  // For SPL tokens: /token/holders returns top holders with amount + decimals
  // For native SOL: /account/top-holders returns top SOL balance accounts
  try {
    if (!isNative) {
      // SPL token holders via Solscan
      const r = await fetch(
        `https://public-api.solscan.io/token/holders?tokenAddress=${mintAddress}&limit=50&offset=0`,
        { headers: { "Accept": "application/json", "User-Agent": "CryptoRisk/1.0" } }
      );
      if (!r.ok) throw new Error(`Solscan HTTP ${r.status}`);
      const d = await r.json();

      // Solscan also returns token total supply in the same call
      const tokenInfo = await fetch(
        `https://public-api.solscan.io/token/meta?tokenAddress=${mintAddress}`,
        { headers: { "Accept": "application/json" } }
      ).then(r2 => r2.ok ? r2.json() : {}).catch(() => ({}));

      totalSupply = parseFloat(tokenInfo.supply || 0) / Math.pow(10, tokenInfo.decimals || 0);

      const rawHolders = d?.data || [];
      if (rawHolders.length) {
        holders = rawHolders.map(h => {
          const amount = parseFloat(h.amount || 0) / Math.pow(10, h.decimals || tokenInfo.decimals || 0);
          const pct    = totalSupply > 0 ? (amount / totalSupply) * 100 : 0;
          const known  = KNOWN_SOL_WALLETS[h.address] || {};
          return {
            address:    h.address,
        ...
        source = "Solscan public API (live)";
      }

      // Enrich meta from token info
      if (!meta.name && tokenInfo.name) {
        meta.name   = tokenInfo.name;
        meta.ticker = tokenInfo.symbol?.toUpperCase() || meta.ticker;
        meta.image  = tokenInfo.icon || meta.image;
      }

    } else {
      // Native SOL — Solscan top-holders endpoint
      const r = await fetch(
        `https://public-api.solscan.io/account/top-holders?limit=50`,
        { headers: { "Accept": "application/json", "User-Agent": "CryptoRisk/1.0" } }
      );
      if (!r.ok) throw new Error(`Solscan SOL holders HTTP ${r.status}`);
      const d = await r.json();

      // CoinGecko for SOL circulating supply
      ...
      } catch { meta.name = "Solana"; meta.ticker = "SOL"; }

      totalSupply = circulatingSOL;
      const rawHolders = d?.data || [];
      if (rawHolders.length) {
        holders = rawHolders.map(h => {
          const lamports = parseFloat(h.lamports || h.balance || 0);
          const sol      = lamports / 1e9;
          const pct      = (sol / totalSupply) * 100;
          const known    = KNOWN_SOL_WALLETS[h.account] || {};
          return {
            address:    h.account,
            percentage: parseFloat(pct.toFixed(4)),
    ...
    console.warn("[Solscan]", e.message);
  }

  // ── Source 2: Solana public RPC — getTokenLargestAccounts (no key) ────────
  // Works for any SPL token. Returns top 20 accounts only.
  // For native SOL this RPC method doesn't apply, so only used for SPL tokens.
  if (!holders.length && !isNative) {
    try {
      const rpcUrl = "https://api.mainnet-beta.solana.com";
      const [acctResp, supplyResp] = await Promise.all([
        fetch(rpcUrl, {
          method: "POST", headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ jsonrpc:"2.0", id:1,
            method:"getTokenLargestAccounts", params:[mintAddress] }),
        }),
        fetch(rpcUrl, {
          method: "POST", headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ jsonrpc:"2.0", id:2,
            method:"getTokenSupply", params:[mintAddress] }),
        }),
      ]);
      ...
      const accounts = acctData?.result?.value || [];
      ...
      if (accounts.length) {
        ...
        source = "Solana public RPC (live)";
      }
    } catch (e) {
      console.warn("[Solana RPC]", e.message);
    }
  }

  // ── Source 3: Helius (optional, only if key is set) ────────────────────────
  // If neither Solscan nor public RPC worked AND the user has set HELIUS_API_KEY,
  // use it as a last resort. Does NOT throw if key is missing.
  if (!holders.length) {
  ...
  if (!holders.length)
    throw new Error(
      ...
      "Solscan and the public Solana RPC both failed to respond. Please try again in a moment."
    );
