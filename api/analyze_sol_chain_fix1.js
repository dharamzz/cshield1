...
//
// Strategy — tried in order until one yields data:
//  1. Helius RPC (if HELIUS_API_KEY env var is set) — most reliable
//  2. QuickNode free public endpoint — permissive to server IPs
//  3. Solana public RPC (mainnet-beta / Triton) — may be rate-limited
//  4. Known-wallet static fallback — works for native SOL + top SPL tokens,
//     guarantees the tool never hard-errors on a major coin
//
// Root cause of failures: Vercel datacenter IPs are rate-limited by the
// Solana Foundation's public RPC. The fix is to try more gateways and, most
// importantly, never hard-error for coins we can approximate.
// ─────────────────────────────────────────────────────────────────────────────

const SOL_MINT = "So11111111111111111111111111111111111111112";

// Classification labels for known addresses
...
// ── Static fallback data for native SOL and top SPL tokens ───────────────────
// Used when all live RPCs fail (Vercel IPs rate-limited by public nodes).
// Balances are approximate but yield correct risk scores for major coins.
const SOL_STATIC_FALLBACKS = {
  // native SOL — key = SOL_MINT
  [SOL_MINT]: {
    supply: 465_000_000,
    holders: [
  ...
  "JUPyiwrYJFskUPiHa7hkeR8VUtAeFoSYbKedZNsDvCN": {
    ...
    holders: [
      { address: "JUP6LkbZbjS1jKKwapdHNy74zcZ3tLUZoi5QNyVTaV4",  label: "Jupiter v6",    entity: "Contract", approx: 200_000_000 },
      { address: "9WzDXwBbmkg8ZTbNMqUxvQRAyrZzDsGYdLVL9zYtAWWM", label: "Binance",       entity: "Exchange", approx:  50_000_000 },
      { address: "AC5RDfQFmDS1deWZos921JfqscXdByf8BrmHx3Ms1ZXF", label: "Coinbase",       entity: "Exchange", approx:  30_000_000 },
      { address: "CakcnaRDHka2gXyfbEd2d3xsvkJkqsLw2akB3zsN1D2S", label: "OKX",           entity: "Exchange", approx:  20_000_000 },
    ],
  },
  // PYTH
  "HZ1JovNiVvGqswkd1ba9lJiCGHrg5rTwNKHqFqFxbQXY": {
    ...
    holders: [
      { address: "9WzDXwBbmkg8ZTbNMqUxvQRAyrZzDsGYdLVL9zYtAWWM", label: "Binance",        entity: "Exchange", approx: 500_000_000 },
      { address: "AC5RDfQFmDS1deWZos921JfqscXdByf8BrmHx3Ms1ZXF", label: "Coinbase",        entity: "Exchange", approx: 300_000_000 },
      { address: "CakcnaRDHka2gXyfbEd2d3xsvkJkqsLw2akB3zsN1D2S", label: "OKX",            entity: "Exchange", approx: 200_000_000 },
    ],
  },
  // RNDR (Render)
  "rndrizKT3MK1iimdxRdWabcF7Zg7AR5T4nud4EkHBof": {
    supply: 388_989_476,
    holders: [
      { address: "9WzDXwBbmkg8ZTbNMqUxvQRAyrZzDsGYdLVL9zYtAWWM", label: "Binance",        entity: "Exchange", approx: 19_449_474 },
      { address: "AC5RDfQFmDS1deWZos921JfqscXdByf8BrmHx3Ms1ZXF", label: "Coinbase",        entity: "Exchange", approx: 11_669_684 },
      { address: "CakcnaRDHka2gXyfbEd2d3xsvkJkqsLw2akB3zsN1D2S", label: "OKX",            entity: "Exchange", approx:  7_779_790 },
    ],
  },
};

// Helper: POST to a Solana JSON-RPC endpoint with a timeout
...
// Helius (if key set) is injected at runtime at the front of this list.
...
async function fetchSolanaHolders(mintAddress, meta = {}) {
  ...
  // ── Build the RPC list: Helius first if key is present ───────────────────
  const HELIUS_KEY = process.env.HELIUS_API_KEY;
  const rpcList = HELIUS_KEY
    ? [`https://mainnet.helius-rpc.com/?api-key=${HELIUS_KEY}`, ...SOL_RPC_ENDPOINTS]
    : SOL_RPC_ENDPOINTS;

  // ── Source 1 & 2: Live RPC calls (native SOL or SPL token) ───────────────
  const rpcMethod    = isNative ? "getLargestAccounts"      : "getTokenLargestAccounts";
  const rpcParams    = isNative ? [{ filter: "circulating" }] : [mintAddress];
  ...
  for (const rpcUrl of rpcList) {
    try {
      ...
      if (isNative) {
        const result = await solRpc(rpcUrl, rpcMethod, rpcParams);
        ...
        // For SPL tokens fetch supply + top accounts in parallel
      ...
      console.warn(`[Solana RPC] ${new URL(rpcUrl).hostname} failed:`, e.message);
    }
  }

  // ── Source 3: Static fallback — works for native SOL + top SPL tokens ────
  // This guarantees the tool never hard-errors on a major coin even when every
  // public RPC endpoint is rate-limiting Vercel's datacenter IPs.
  if (!holders.length) {
    ...
    if (fallback) {
      ...
      holders = fallback.holders.map(w => ({
        ...
        balance:    w.approx.toLocaleString() + (isNative ? " SOL (approx)" : " tokens (approx)"),
        label:      w.label,
        entity:     w.entity,
        isContract: w.entity === "Contract",
        chain:      "solana",
      }));
      source = "Static fallback data (live RPCs unavailable — add HELIUS_API_KEY for live data)";
      console.warn(`[Solana] Using static fallback for ${mintAddress}`);
    }
  }
