...
//  1. Helius RPC  — if HELIUS_API_KEY env var is set (most reliable, free tier)
//  2. Public Solana RPC pool — tried with 7s timeout each; often rate-limited
//     from Vercel datacenter IPs but worth trying first
//  3. SOL_STATIC_FALLBACKS — hardcoded approximate data for native SOL + top
//     SPL tokens. Guarantees the tool never hard-errors on a major coin.
//
// Add a free HELIUS_API_KEY at helius.dev → Vercel env vars for live data.
// ─────────────────────────────────────────────────────────────────────────────

const SOL_MINT = "So11111111111111111111111111111111111111112";

// Address → label/entity for classification
...
// Static fallback holder data — fires when all live RPCs are rate-limiting.
// Keyed by mint address. Approximate balances; HELIUS_API_KEY gives live data.
...
// POST to a Solana RPC endpoint with a hard abort timeout
...
async function fetchSolanaHolders(mintAddress, meta = {}) {
  ...
  // ── CoinGecko: SOL supply + metadata (best-effort, not required) ──────────
  ...
  // ── Build RPC list: Helius first if key is set ────────────────────────────
  ...
  // ── Static fallback — native SOL + top SPL tokens ─────────────────────────
  if (!holders.length) {
    const fb = SOL_STATIC_FALLBACKS[mintAddress];
    if (fb) {
      totalSupply = isNative ? circulatingSOL : fb.supply;
      holders = fb.holders.map(w => ({
        address:    w.address,
        percentage: parseFloat(((w.approx / totalSupply) * 100).toFixed(4)),
        balance:    `${w.approx.toLocaleString()} ${fb.unit} (approx)`,
        label:      w.label,
        entity:     w.entity,
        isContract: w.entity === "Contract",
        chain:      "solana",
...
//
// Well-known coins skip the /search call entirely (saves one CoinGecko token).
// All CoinGecko calls retry once on 429 after a 1.2 s pause.
// ─────────────────────────────────────────────────────────────────────────────

// ticker/name → { id, solana? } — bypasses /search for popular coins
const KNOWN_COIN_IDS = {
  // Solana SPL tokens — resolved directly to fetchSolanaHolders
...
async function resolveByName(query) {
  ...
  // ── Fast path: known Solana SPL token ────────────────────────────────────
  if (known?.solana) {
    const meta = {};
    try {
    ...
    } catch { /* metadata optional — holders still load from static fallback */ }
    return fetchSolanaHolders(known.solana, meta);
  }

  // ── Fast path: known native coin ─────────────────────────────────────────
  if (known) {
    const id = known.id;
