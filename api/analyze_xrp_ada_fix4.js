...
//  DOGE         dogechain.info               /api/v1/address/balance/<addr>
//  ADA          Static known-wallet list     (no free Cardano richlist API)
//  HYPE         Static known-wallet list     (Hyperliquid richlist not yet public)
...
// ─────────────────────────────────────────────────────────────────────────────
// XRP — XRPL public API (free, no key)
// Uses xrplcluster.com WebSocket-compatible REST endpoint.
// Falls back to known large wallet list with static balances.
// ─────────────────────────────────────────────────────────────────────────────

const KNOWN_XRP_WALLETS = [
  { address: "rHb9CJAWyB4rj91VRWn96DkukG4bwdtyTh",  label: "Genesis account",     entity: "Whale"    },
  { address: "rEhKZcz9WhPCFSWRNp1JQBpQrRSMRxKoAb",  label: "Ripple escrow",       entity: "Contract" },
  { address: "rN7n3473SaZBCG4dFL75SWvBaQkjrPLsF",   label: "Ripple 1",            entity: "Contract" },
  { address: "rPT1Sjq2YGrBMTttX4GZHjKu9dyfzbpAYe",  label: "Binance",             entity: "Exchange" },
  { address: "rEy8TFcrAPvhpKrwyrscNYyqBGUkU9dkjX",  label: "Coinbase",            entity: "Exchange" },
  { address: "rLNaPoKeeBjZe2qs6x52yVPZpZ8td4dc6w",  label: "Bitstamp",            entity: "Exchange" },
  { address: "rUobPBEFBqnFCZnDXtmrBLFaFvBqYbsvgE",  label: "Kraken",              entity: "Exchange" },
  { address: "rJb5KsHsDHF1YS5B5DU6QCkH5NsPaKQTcy",  label: "Huobi/HTX",           entity: "Exchange" },
  { address: "rKiCet8SdvWxPXnAgYarFUXMh1zCPz432Y",  label: "OKX",                 entity: "Exchange" },
  { address: "rfEd2pnb2cBByjLBQzrpoZqCQAWBPXZQDS",  label: "Unknown whale",       entity: "Whale"    },
  { address: "r3yWpFRNLvfgr7JVKoKFE5FfFzrFBHsYUq",  label: "Unknown whale",       entity: "Whale"    },
  { address: "r4GDFMLGJUKMjNEycM3FMRqHQgX5xoGCg",   label: "Unknown whale",       entity: "Whale"    },
];

async function fetchXRPHolders(meta = {}) {
  let holders = [];
  let totalSupply = 100_000_000_000; // XRP max supply
  let source = "";

  // Source 1: XRPL public cluster — ledger top accounts
  try {
    const r = await fetch("https://xrplcluster.com/", {
      ...
      body: JSON.stringify({
        method: "ledger",
        params: [{ ledger_index: "validated", accounts: false, full: false, transactions: false, expand: false }],
      }),
      signal: AbortSignal.timeout(7000),
    });
    // XRPL doesn't have a free richlist — fall through to known wallets
  } catch (_) { /* expected */ }

  // Source 2: Fetch live balances for known large wallets via XRPL REST
  try {
    const results = await Promise.allSettled(
      KNOWN_XRP_WALLETS.map(w =>
        fetch(`https://xrplcluster.com/`, {
          method:  "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            method: "account_info",
            params: [{ account: w.address, ledger_index: "validated" }],
          }),
        ...
        .then(d => ({
          ...w,
          balance: parseFloat(d?.result?.account_data?.Balance || 0) / 1_000_000,
        }))
      )
    );
    const live = results
    ...
    if (live.length >= 3) {
      holders = live.map(w => ({
        address:    w.address,
        percentage: parseFloat(((w.balance / totalSupply) * 100).toFixed(4)),
        balance:    w.balance.toLocaleString(undefined, { maximumFractionDigits: 0 }) + " XRP",
        label:      w.label,
        entity:     w.entity,
        isContract: w.entity === "Contract",
        chain:      "xrp",
  ...
  if (!holders.length) {
    const STATIC = [
      { address: "rHb9CJAWyB4rj91VRWn96DkukG4bwdtyTh",  label: "Genesis account",     entity: "Whale",    approx: 10_000_000_000 },
    ...
    holders = STATIC.map(w => ({
      address:    w.address,
      percentage: parseFloat(((w.approx / totalSupply) * 100).toFixed(4)),
      balance:    w.approx.toLocaleString() + " XRP (approx)",
      label:      w.label,
      entity:     w.entity,
      isContract: w.entity === "Contract",
      chain:      "xrp",
  ...
  try {
    ...
    totalSupply    = cg.market_data?.circulating_supply || totalSupply;
    meta.price     = meta.price     || cg.market_data?.current_price?.usd;
    meta.marketCap = meta.marketCap || cg.market_data?.market_cap?.usd;
    meta.image     = meta.image     || cg.image?.small;
    // Recompute percentages with real supply
    holders = holders.map(h => ({
      ...h,
      percentage: parseFloat(((parseFloat(h.balance) / totalSupply) * 100).toFixed(4)),
    }));
  } catch { /* use defaults */ }

  return {
    coin: {
      name: "XRP", ticker: "XRP", address: null,
      chain: "xrp", chainLabel: "XRP Ledger",
      source, ...meta,
    },
    holders: holders.sort((a, b) => b.percentage - a.percentage).slice(0, 50),
  };
...
// ─────────────────────────────────────────────────────────────────────────────
// DOGECOIN — dogechain.info free API + known large wallet fallback
// ─────────────────────────────────────────────────────────────────────────────

const KNOWN_DOGE_WALLETS = [
  ...
  { address: "DQkwDpRYUyNNnoEZDfSGFFeQvLgbdEXiNd",  label: "Unknown whale",    entity: "Whale",    approx:  7_000_000_000 },
...
async function fetchDOGEHolders(meta = {}) {
  let holders = [];
  let totalSupply = 144_000_000_000; // approximate circulating
  let source = "";

  // Source 1: dogechain.info balance API for known large wallets
  try {
    const results = await Promise.allSettled(
      KNOWN_DOGE_WALLETS.map(w =>
        fetch(`https://dogechain.info/api/v1/address/balance/${w.address}`, {
          signal: AbortSignal.timeout(6000),
        })
        .then(r => r.json())
        .then(d => ({ ...w, balance: parseFloat(d?.balance || 0) }))
      )
    );
    const live = results
      .filter(r => r.status === "fulfilled" && r.value.balance > 0)
    ...
    if (live.length >= 3) {
      holders = live.map(w => ({
        address:    w.address,
        percentage: parseFloat(((w.balance / totalSupply) * 100).toFixed(4)),
        balance:    w.balance.toLocaleString(undefined, { maximumFractionDigits: 0 }) + " DOGE",
        label:      w.label,
        entity:     w.entity,
        isContract: false,
        chain:      "dogecoin",
  ...
  if (!holders.length) {
    holders = KNOWN_DOGE_WALLETS.map(w => ({
      address:    w.address,
      percentage: parseFloat(((w.approx / totalSupply) * 100).toFixed(4)),
      balance:    w.approx.toLocaleString() + " DOGE (approx)",
      label:      w.label,
      entity:     w.entity,
      isContract: false,
      chain:      "dogecoin",
  ...
  try {
    const cg = await fetchJSON(
      "https://api.coingecko.com/api/v3/coins/dogecoin?localization=false&tickers=false&market_data=true&community_data=false&developer_data=false"
    );
    totalSupply    = cg.market_data?.circulating_supply || totalSupply;
    meta.price     = meta.price     || cg.market_data?.current_price?.usd;
    meta.marketCap = meta.marketCap || cg.market_data?.market_cap?.usd;
  ...
  return {
    coin: {
      name: "Dogecoin", ticker: "DOGE", address: null,
      chain: "dogecoin", chainLabel: "Dogecoin",
      source, ...meta,
    },
    holders: holders.sort((a, b) => b.percentage - a.percentage).slice(0, 50),
  };
...
// ─────────────────────────────────────────────────────────────────────────────
// CARDANO (ADA) — Blockfrost-free-tier + static fallback
// Blockfrost free tier requires no credit card (1M requests/day).
// If BLOCKFROST_KEY is not set, we use the static known-wallet list.
// ─────────────────────────────────────────────────────────────────────────────

async function fetchADAHolders(meta = {}) {
  let holders = [];
  let totalSupply = 35_000_000_000;
  let source = "";

  // Source 1: Blockfrost if key is set
  const BLOCKFROST_KEY = process.env.BLOCKFROST_KEY;
  if (BLOCKFROST_KEY) {
    try {
      const r = await fetch("https://cardano-mainnet.blockfrost.io/api/v0/addresses/stake1u9x39hrfagrfkjexzgmjh0hzhdvtmhlvlzn5ywj6e46wqsd6v5k2/total", {
        headers: { project_id: BLOCKFROST_KEY },
        signal: AbortSignal.timeout(6000),
      });
      // Blockfrost doesn't have a free richlist, so fall through
    } catch (_) { /* fall through */ }
  }

  // Source 2: Static known-wallet fallback (always used — no free richlist API exists)
  const STATIC = [
  ...
  source = "Known ADA wallet list (approximate — no free Cardano richlist API exists)";

  // CoinGecko metadata
  try {
    const cg = await fetchJSON(
      "https://api.coingecko.com/api/v3/coins/cardano?localization=false&tickers=false&market_data=true&community_data=false&developer_data=false"
    );
    totalSupply    = cg.market_data?.circulating_supply || totalSupply;
    meta.price     = meta.price     || cg.market_data?.current_price?.usd;
    meta.marketCap = meta.marketCap || cg.market_data?.market_cap?.usd;
  ...
  return {
    coin: {
      name: "Cardano", ticker: "ADA", address: null,
      chain: "cardano", chainLabel: "Cardano",
      source, ...meta,
    },
    holders: holders.sort((a, b) => b.percentage - a.percentage).slice(0, 50),
  };
...
// ─────────────────────────────────────────────────────────────────────────────
// HYPE (Hyperliquid) — native coin of Hyperliquid L1
// No public richlist API exists yet; uses static known-wallet approximation.
// ─────────────────────────────────────────────────────────────────────────────

async function fetchHYPEHolders(meta = {}) {
  let totalSupply = 1_000_000_000; // HYPE total supply
  let source = "";

  // Hyperliquid has a public info API — try fetching top validator/staking info
  let holders = [];
  try {
    ...
    // Response doesn't include holder data directly — fall through to static
  } catch (_) { /* fall through */ }

  // Static known-wallet fallback
  const STATIC = [
    ...
    { address: "0xBE0eB53F46cd790Cd13851d5EFf43D12404d33E8", label: "Unknown whale",         entity: "Whale",    approx:  15_000_000 },
  ];
  ...
  source = "Known HYPE wallet list (approximate — Hyperliquid richlist API not yet public)";
  ...
  return {
    coin: {
      name: "Hyperliquid", ticker: "HYPE", address: null,
      chain: "hyperliquid", chainLabel: "Hyperliquid L1",
      source, ...meta,
    },
    holders: holders.sort((a, b) => b.percentage - a.percentage).slice(0, 50),
  };
